...
String names[]={"Vbatt","tempSHT","humSHT","pressBMP","tempW","pressW","Hwat"}; //update this list if you add sensors!!!
...
String Sensors::getFileData () { 
  String datastring = String(values[0],1) + String(";") +String(values[1],3) +String(";") +String(values[2],2) + String(";") +String(values[3],2) + String(";")+String(values[4],2) + String(";") +String(values[5],2) + String(";") +String(values[6],3) + String(";");
  return datastring;
}
...
void Sensors::measure() {
  ...
  //connect and start the SHT35 PCB sensor 
    ...
    //measure pressure of Keller sensor
  ...
  values[4]=waterTempertureC;
  values[5]=waterPressureBar*1000;
  values[6]=waterDepthM*100;
  Serial1.end();
  ...
  //values[3]=data.temperature; //tempBMP pas utile si SHT temp
