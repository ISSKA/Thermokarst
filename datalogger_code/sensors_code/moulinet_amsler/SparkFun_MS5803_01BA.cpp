...
// Return a pressure reading units Pa.
{
  ...
  pressure_reported = pressure_reported / 100.0f;
...

{
  ...
  { 
    T2 = ((int64_t)dT * dT) >> 33;
    OFF2 = 3 * (temp_calc - 2000) * (temp_calc - 2000);
    SENS2 = 7 * (temp_calc - 2000) * (temp_calc - 2000);
    
    if(temp_calc < -1500)
    // If temp_calc is below -15.0C 
    {
      SENS2 = SENS2 + 2 * ((temp_calc + 1500) * (temp_calc + 1500));
    }
    } 
  else
  ...
  { 
    ...
    if(temp_calc >= 4500)
    {
      SENS2 = SENS2 - ((temp_calc -4500) * (temp_calc -4500)) / 8;
    }
  }
